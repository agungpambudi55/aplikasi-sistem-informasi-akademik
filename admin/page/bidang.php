...
if($user['akses']=="1")
{?>
    <div id="btn1" onClick="pages('bidang_tambah.php');"></div>
    <p onclick="tanya('bidang');" id="hapus_semua"></p>
<?php 
}
$p=que("select ket1 from tb_aksesoris where nama='paging'");
$page		= fetch($p);
$perpage	= $page['0']; 
$querytotal	= que("SELECT * FROM tb_bidang");
...
{?><p onClick="pg('bidang.php?page=<?php echo $curpage+1; ?>')" id="next_a"></p> <?php }	
else
{echo "<p  id='next_d'></p>";}	
?><select name='halaman' onchange="pg('bidang.php?page='+this.value)"><?php
for($r=1; $r <= $jumpage ; $r++)
	{	
		if($curpage == $r)
		{
...
{?><p onClick="pg('bidang.php?page=<?php echo $curpage-1; ?>')" id="back_a"></p> <?php }
else
{echo "<p  id='back_d'></p>";}	
...
<?php
$qry = que("SELECT * FROM tb_bidang ORDER BY nama_bidang ".$limit);
if(num($qry)==0)
...
";?>		<div id="edit" onClick="pages('bidang_edit.php?id=<?php echo $out['0'];?>')"></div> <?php echo "
";?>		<div id="hapus" onClick="konfirm('menghapus <?php echo $out['1'];?>','bidang_hapus.php?id=<?php echo $out['0'];?>');"></div> <?php echo "
		</td>";
}
echo "
	</tr>
...
{
if($_GET['cr']=="")
{?><script>pages('bidang.php?notif=ta');</script><?php }
else
{
	?>
	<div id="paging">
	<p onClick="pages('bidang.php')" id="back"></p>
<?php if($user['akses']=="1")
{?><p onclick="tanya('bidang');" id="hapus_semua"></p><?php  }?>
	</div>
	<table width="100%" id="data">
		<tr>
<?php if($user['akses']=="1"){?><th width='5%'><input type="checkbox" id="cek" onClick="cek_all()" value="All"></th><?php }?>
...
<?php 
	...
	<?php
	$qrycr = que("SELECT * FROM tb_bidang WHERE kode_bidang LIKE '%$_GET[cr]%' OR nama_bidang LIKE '%$_GET[cr]%' ORDER BY nama_bidang asc");
	if(num($qrycr)==0)
...
{
echo "	<td align='center'>
	";?>		<div id="edit" onClick="pages('bidang_edit.php?id=<?php echo $cr['0'];?>')"></div> <?php echo "
	";?>		<div id="hapus" onClick="konfirm('menghapus <?php echo $cr['1'];?>','bidang_hapus.php?id=<?php echo $cr['0'];?>')"></div> <?php echo "
			</td>";
}
echo "
		</tr>
